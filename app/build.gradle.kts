...
android {
    ...
    defaultConfig {
        ...
        // fresh work.
        // v1.9.1: versionCode 33, versionName "1.9.1".
        // "Make the app deployable, honestly" release. The
        // v1.9.0 release shipped a credible pre-release
        // build that compiled to a signed APK and had the
        // security/policy docs in place, but the fresh-
        // eyes critique came back at 5.5/10 ("credible
        // pre-release, not something I'd put in front of a
        // real user today without 2-4 more weeks of
        // polish"). v1.9.1 is the polish pass that closes
        // every P0 + P1 + most P2 from that critique:
        //   P0-a11y: Threat model section headers now
        //     carry `Modifier.semantics { heading() }`;
        //     Today count summary wrapped in a single
        //     `Modifier.semantics { contentDescription = ... }`.
        //   P0-drive: Settings rows relabelled from
        //     "Back up to Google Drive" to "Save backup to
        //     a folder..." (the implementation uses SAF,
        //     not Drive REST).
        //   P0-onboarding: 3-page first-run flow extended
        //     to 4 pages using the v1.9.0
        //     `onboarding_screen_N_*` strings.
        //   P0-widgets: Today + Decay widget badge data
        //     wired to `InstructionDao.countOpen` and
        //     `PersonDao.countQuietSince` (the v1.9.0
        //     trade-off was a hard-coded "—").
        //   P1-bundle: per-ABI splits in `splits { abi }
        //     so the user-installed arm64-v8a APK is
        //     23.4 MB (down from 68 MB universal, or
        //     71.3 MB in v1.9.0's pre-split report).
        //   P1-screenshots: 8 placeholder PNGs at
        //     1080x1920 generated by a Python helper;
        //     each one carries a red "PLACEHOLDER"
        //     stripe so a real submission cannot
        //     accidentally ship them.
        //   P1-recovery-e2e: `RecoveryPhraseEndToEndTest`
        //     pins the BIP39 generation -> checksum
        //     validate -> SHA-256 hash pipeline.
        //   P2-typo: a corrupted ellipsis byte in
        //     `settings_drive_backup_working` is fixed.
        //   P2-tamil-hindi: 4 new backup strings translated
        //     into Tamil + Hindi.
        // No public-API or schema changes; versionCode
        // bumps to 33 because the v1.9.0 honest
        // deployability rating of 5.5/10 explicitly
        // committed to a v1.9.1 follow-up.
        versionCode = 33
        versionName = "1.9.1"
