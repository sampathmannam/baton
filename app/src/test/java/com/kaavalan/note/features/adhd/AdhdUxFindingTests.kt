package com.kaavalan.note.features.adhd
...
class AdhdUxFindingTests {
    ...
    fun `3 carried over is the only silent rollover status`() {
        ...
        val brief = newBriefGenerator().build(
            type = com.kaavalan.note.data.brief.BriefType.MORNING,
            date = today,
    ...
    fun `5 only three top-level routes are defined`() {
        val routes = listOf(
            com.kaavalan.note.Routes.HOME,
            com.kaavalan.note.Routes.TODAY,
        )
        ...
        assertTrue(
            ...
            com.kaavalan.note.Routes.PERSON.startsWith("person/"),
        )
    }
