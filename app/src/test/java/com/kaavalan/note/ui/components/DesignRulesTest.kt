...
class DesignRulesTest {
    ...
    private fun uiSources(): List<File> {
        val root = File("src/main/java/com/kaavalan/note")
        ...
        uiSources().forEach { f ->
            ...
            listOf("overdue", "missed").forEach { word ->
                ...
                if (regex.containsMatchIn(scannable)) {
                    offenders.add("${f.relativeTo(File("src/main/java/com/kaavalan/note"))}: $word")
                }
            }
        }
    ...
    fun rule2_noWhatsNewModalInUi() {
        ...
        // line.
        val offenders = mutableListOf<String>()
        val mainFile = File("src/main/java/com/kaavalan/note/MainActivity.kt")
        ...
        uiSources().forEach { f ->
            ...
            listOf("feature of the day", "tip of the day", "tips tab").forEach { phrase ->
                if (text.contains(phrase, ignoreCase = true)) {
                    offenders.add("${f.relativeTo(File("src/main/java/com/kaavalan/note"))}: $phrase")
                }
            }
        }
        if (offenders.isNotEmpty()) {
    ...
    fun rule8_noMagicStringSelfDestructTimers() {
        ...
        uiSources().forEach { f ->
            ...
            if (regex.containsMatchIn(text)) {
                offenders.add(f.relativeTo(File("src/main/java/com/kaavalan/note")).path)
            }
        }
        if (offenders.isNotEmpty()) {
            throw AssertionError(
