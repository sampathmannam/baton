...
/**
 * v2.0 Tier 2 (data layer) regression test for the v10 -> v11
 * migration. We exercise the same SQL strings the production
 * migration runs (mirrored from the [com.baton.app.data.local.AppDatabase.MIGRATION_10_11]
 ...
 * would require us to stand up the full v10 entity graph
 * (8 entities + cross-refs) just to assert the migration SQL,
...
class Migration10To11Test {
    ...
    fun `migration SQL adds Tier 2 columns to persons`() {
        buildMinimalV10AndApplyMigration()
        SQLiteDatabase.openOrCreateDatabase(v10DbPath, null).use { db ->
    ...
    fun `migration SQL urgency defaults to normal on existing instructions`() {
        SQLiteDatabase.openOrCreateDatabase(v10DbPath, null).use { db ->
            seedV10PersonsAndInstructionsAndCaptures(db)
            db.execSQL(
    ...
    private fun seedV10PersonsAndInstructionsAndCaptures(db: SQLiteDatabase) {
        db.execSQL(
            """
    ...
    /**
     * Build a minimal v10 schema covering persons, instructions,
     * captures, then run the migration SQL directly.
     */
    private fun buildMinimalV10AndApplyMigration() {
        SQLiteDatabase.openOrCreateDatabase(v10DbPath, null).use { db ->
            db.execSQL(
                """
                CREATE TABLE persons (
