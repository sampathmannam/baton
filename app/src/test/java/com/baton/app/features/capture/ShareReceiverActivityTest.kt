...
class ShareReceiverActivityTest {
    ...
    fun `activity is package-consistent with the manifest`() {
        ...
        assertEquals(
            "com.baton.app.features.capture.ShareReceiverActivity",
            activityClass.name,
        )
    }
    ...
    fun `translucent theme file is on the source classpath`() {
        ...
        assertTrue(
            "themes.xml must declare Theme.Baton.Translucent.NoDisplay",
            "Theme.Baton.Translucent.NoDisplay" in contents,
        )
    }
}
