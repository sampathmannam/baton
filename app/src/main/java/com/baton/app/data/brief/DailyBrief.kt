...
internal fun Instruction.needsYouToday(today: java.time.LocalDate, now: java.time.Instant): Boolean {
    if (direction !in setOf(com.baton.app.data.instructions.Direction.INCOMING, com.baton.app.data.instructions.Direction.SELF)) return false
    ...
    if (priority == com.baton.app.data.instructions.Priority.HIGH && status == Status.OPEN) return true
...
internal fun Instruction.waitingOnOthers(): Boolean =
    direction == com.baton.app.data.instructions.Direction.OUTGOING &&
